...
function CreateLesson() {
  ...
  useEffect(() => {
    ...
    if (location.state) {
      ...
      if (location.state.quiz !== undefined) setQuiz(location.state.quiz);
    }

    if (params.id && !lessonSet) {
      api
        .get(`/Lesson/GetLessonDetails/${params.id}`)
        .then((res) => {
          setLesson(res.data);
        })
        .catch((err) => {
          if (err.response.status == 404) navigate("/moderator/lessons");
  ...
  useEffect(() => {
    setCategory(
      categories.find((c: Category) => c.id === lesson.categoryId)?.name ?? ""
    );
  }, [categories, lesson]);
      ...
      .then(() => {
        api
          ...
          .then((res) => {
            ...
            setLesson((prev) => ({
              ...prev,
              quizId: res.data !== "" ? res.data : undefined,
            }));
  ...
  return (
    <div className="studio-create">
      <IoMdArrowRoundBack
        ...
        onClick={() => navigate("/moderator/lessons")}
      />
          ...
          <div className="form-field">
            ...
            <div className="studio-upload-con">
              <button
                type="button"
                onClick={() =>
                  navigate("/moderator/lesson-editor", {
                    state: {
                      lesson: lesson,
