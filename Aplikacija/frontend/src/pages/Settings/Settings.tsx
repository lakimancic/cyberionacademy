...
function Settings() {
    ...
    const onSubmitBasic = (values: BasicInfo) => {
        ...
        api.put("/Account/ChangeInfo", values)
            .then(() => {
                setOrigBasic(values);
    ...
    const onSubmitPassword = (values: PasswordsInfo) => {
        ...
        api.put("/Account/ChangePassword", values)
            .then(() => {
            ...
            <form className="settings-form settings-basic" onSubmit={basicForm.handleSubmit(onSubmitBasic)}>
                ...
                <div className="form-field">
                    ...
                    <Select
                        ...
                        onChange={event => {
                            basicForm.trigger();
                            setBasicDiffer(isBasicDiffer({...basicForm.getValues(), country: event.target.value}))
                        }}
                    >
                        {countries.map((c) => (
                ...
                >{basicLoading ? <CircularProgress color='inherit' size="1.6rem"/> : 'Save Changes'}</button>
            </form>
            <form className="settings-form settings-sec" onSubmit={passwordForm.handleSubmit(onSubmitPassword)}>
                <h2>Security Information</h2>
                <p className={`settings-error ${passwordSuccess.length > 0 ? 'settings-success' : ''}`}>
                ...
                >{passwordLoading ? <CircularProgress color='inherit' size="1.6rem"/> : 'Update Password'}</button>
            </form>
            <Footer />
        </div>
    )
