...
function HomePage() {
  ...
  const mappedReccoms = useMemo(
    () =>
      recomms.map((r, i) => (
        <div className="home-recomm" key={i}>
          <h2>{r.label}</h2>
          {r.array.map((item, ii) => (
            <div
              className="home-recomm-item"
              key={ii}
              onClick={() => navigate(`/${r.type}s/${item.id}`)}
            >
              <img src={(categories as any)[item.categoryShort]} />
              <div className="challenge-name">
                <strong>
                  {r.type === "challenge" ? item.name : item.title}
                </strong>
  ...
  return (
    <div className="home-page">
      <h1>
        Cyberion<span>Academy</span>
      </h1>
      <blockquote style={{ opacity: opacity }}>
        "{quote.quote}"<p className="home-quote-author">- {quote.author}</p>
      </blockquote>
      <h2>
        <MdKeyboardArrowLeft
        ...
        <MdKeyboardArrowRight
          ...
          onClick={() =>
            setRIndex((prev) => Math.min(recomms.length - 3, prev + 1))
          }
        />
      </h2>
      {!loading && (
