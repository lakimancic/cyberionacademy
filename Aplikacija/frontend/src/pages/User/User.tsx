...
function User() {
  ...
  useEffect(() => {
    api.get(`/User/${userId}/Info`).then((resp) => {
      setUserInfo(resp.data);
    });

    api.get(`/User/${userId}/Stats`).then((resp) => {
      setUserStats(resp.data);
    });

    api.get(`/User/${userId}/ChallengesInfo`).then((resp) => {
      setChalRadarData(resp.data);
    });

    api.get(`/User/${userId}/LessonsInfo`).then((resp) => {
      setQuizRadarData(resp.data);
    });

    api.get(`/User/${userId}/PointsPerMonth`).then((resp) => {
    ...
    api.get(`/User/${userId}/Activity`).then((resp) => {
      ...
      <div className="user-main">
        <div className="user-info">
          <div className="user-avatar">
            <Avatar className="user-profile" src={profilePicture.avatarUrl} />
            <Avatar
              className="user-flag"
              src={
                userInfo?.country
                  ? `https://flagcdn.com/w160/${userInfo.country}.png`
                  : undefined
              }
            >
          ...
          <div className="user-text">
            <p className={`user-role-${userInfo?.role.toLocaleLowerCase()}`}>
              {userInfo?.role}
            </p>
      ...
      <div className="user-year-stats">
        ...
        <LineChart
          dataset={growthData.map((gd) => {
            return {
              yearMonth: `${gd.month}/${gd.year}`,
              points: gd.totalPoints,
            };
          })}
      ...
      <div className="user-radars">
        <div className="user-radar">
          ...
          <RadarChart
            height={400}
            series={[
              { label: "Challenges", data: chalRadarData.map((c) => c.num) },
            ]}
            radar={{
        ...
        <div className="user-radar">
          ...
          <RadarChart
            ...
            series={[
              {
                label: "Lessons (Quizes)",
                data: quizRadarData.map((c) => c.num),
                color: "#34be7eff",
              },
            ]}
            radar={{
