...
function Quiz() {
        ...
        if (q.id in quizSave) {
          if (q.type < 2) {
            q.options = q.options?.map((o) => ({
              ...o,
              isCorrect: quizSave[q.id]?.options?.[o.id].isCorrect ?? false,
            }));
          } else if (q.type === 2) {
              ...
              q.pairs
                ? q.pairs
                    .map((p) => {
                      return {
                        left:
                          q.leftPairs?.findIndex((pp) => pp.left === p.left) ??
                          -1,
                        right:
                          q.rightPairs?.findIndex(
                            (pp) => pp.right === p.right
                          ) ?? -1,
                      };
                    })
  ...
  const fetchQuiz = () => {
    api
      .get("/Quiz/QuizDetails", { params: { id: params.id } })
      .then((res) => {
        setQuiz(res.data);

        if (res.data.doingNow) {
          api
            .get("/Quiz/Continue", { params: { id: res.data.id } })
            .then((resp) => {
              setQuestions(loadAnswers(resp.data.questions));
              setQuiz((prev) =>
                prev ? { ...prev, time: resp.data.time } : prev
              );
            })
            .catch((err) => {
      ...
      <div className="quiz-header">
        <div className="quiz-left">
          ...
          <div className="quiz-title">
            <h1>
              Quiz for:{" "}
              <Link to={`/lessons/${quiz.lessonId}`}>{quiz.title}</Link>
            </h1>
        ...
        <div className="quiz-sidebar">
            ...
            >
              ...
              <div className="action-text">
                ...
                <p className="action-description">
                  {quiz.cooldown
                    ? `You can start quiz in ${quiz.cooldown} days`
          ...
          >
            ...
            <div className="author-info">
              <AuthImage
                src={`/User/${quiz.authorId}/ProfilePicture`}
                element={Avatar}
              />
              <div className="author-details">
                <p
                  className={`author-role role-${quiz.authorRole.toLowerCase()}`}
                >
                  {quiz.authorRole}
                </p>
          ...
          {quiz.results.map((result, ind) => (
            <div className="quiz-attempt" key={ind}>
              <div className="quiz-time-at">
                <p>Started at:</p>
                <strong>
                  {new Date(result.startedAt).toLocaleDateString("en-GB", {
                    day: "numeric",
                    month: "numeric",
                    year: "numeric",
                    hour: "2-digit",
              ...
              <div className="quiz-time-at">
                ...
                <strong>
                  {result.finishedAt
                    ? new Date(result.finishedAt).toLocaleDateString("en-GB", {
                        day: "numeric",
                        month: "numeric",
                        year: "numeric",
                        hour: "2-digit",
              ...
              <div className="quiz-attempt-points">
                <p>Points:</p>
                <strong>
                  {result.points} / {quiz.totalPoints}
                </strong>
              </div>
              <div
                className={`quiz-grade quiz-${getGrade(
                  (result.points * 100) / quiz.totalPoints
                )}`}
              >
