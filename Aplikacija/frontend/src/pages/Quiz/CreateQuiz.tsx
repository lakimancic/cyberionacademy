...
function CreateQuiz() {
  ...
  const removeOption = (qIndex: number, oIndex: number) => {
    ...
    if (questions[qIndex].answers) {
      if (questions[qIndex].answers.length == 2) return;
  ...
  const removePair = (qIndex: number, pIndex: number) => {
    ...
    if (questions[qIndex].pairs) {
      if (questions[qIndex].pairs.length == 2) return;
  ...
  const validateQuiz = async () => {
    ...
    if (
      quiz.questionCount < Math.round(quiz.questions.length / 2) ||
      ...
      setGlobalError(
        `Question Count must be between ${Math.round(
          quiz.questions.length / 2
        )} and ${quiz.questions.length}`
      );
      return false;
    }
  ...
  useEffect(() => {
    if (params.id) {
      api
        .get("/Quiz/ModQuiz", { params: { quizId: params.id } })
        .then((res) => {
          setQuiz(res.data);
        })
        .catch((err) => {
    ...
    <div className="studio-create" ref={containerRef}>
      ...
      {quiz.questions.map((question, qIndex) => (
        ...
        >
          ...
          {questionErrors[qIndex] && (
            <div className="studio-error">{questionErrors[qIndex]}</div>
          )}
          ...
          <div className="form-field">
            <div className="form-label">
              Answers ({typesLabel[question.type]})
            </div>
            {question.type < 3 && (
              <div className="studio-answer-grid">
                  ...
                  question.answers?.map((option, oIndex) => {
                    return (
                      <React.Fragment key={`q-${oIndex}`}>
                        <div className="studio-quiz-answer">
                          <input
                            ...
                            onChange={(oe) => {
                              const questions = quiz.questions;
                              if (questions[qIndex].answers)
                                questions[qIndex].answers[oIndex].text =
                                  oe.target.value;
                              setQuiz((prev) => ({
                                ...prev,
                                questions: questions,
                              ...
                              onChange={(oe) => {
                                ...
                                if (questions[qIndex].answers) {
                                  questions[qIndex].answers.forEach(
                                    (op) => (op.isCorrect = false)
                                  );
                                  questions[qIndex].answers[oIndex].isCorrect =
                                    oe.target.checked;
                                }
                                setQuiz((prev) => ({
                                  ...prev,
                        ...
                        <div
                          className={`studio-quiz-remove ${
                            question.answers?.length == 2
                              ? "studio-quiz-disabled"
                              : ""
                          }`}
                        >
                          <FaMinusCircle
                        ...
                        <div className="studio-quiz-answer">
                          <input
                            ...
                            onChange={(pe) => {
                              const questions = quiz.questions;
                              if (questions[qIndex].pairs)
                                questions[qIndex].pairs[pIndex].left =
                                  pe.target.value;
                              setQuiz((prev) => ({
                                ...prev,
                                questions: questions,
                          ...
                          <input
                            ...
                            onChange={(pe) => {
                              const questions = quiz.questions;
                              if (questions[qIndex].pairs)
                                questions[qIndex].pairs[pIndex].right =
                                  pe.target.value;
                              setQuiz((prev) => ({
                                ...prev,
                                questions: questions,
                ...
                <div
                  className="studio-quiz-add"
                  onClick={() => {
                    if (question.type !== 2) addOption(qIndex);
                    else addPair(qIndex);
                  }}
                >
                  <FaPlusCircle />
      ...
      <div className="studio-add-quiz">
        <Select
          ...
          renderValue={(selected) => {
            if (selected.length === 0) return "Add Question";
      ...
      <div className="studio-create-buttons">
        ...
        <button type="button" className="studio-btn-del" onClick={deleteQuiz}>
          <FaTrashAlt /> Delete
        </button>
      </div>
    </div>
