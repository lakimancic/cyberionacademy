...
function Support({ isNew, isSupport } : Props) {
    ...
    const fetchConversations = async (closed: boolean, page: number) => {
        await api.get("/Support/Conversations", { params: { page: page, closed: closed, forUser: !isSupport } })
            .then(resp => {
                setTotalConvPages(resp.data.totalPages);
                setConversations([...(page > 1 ? conversations : []), ...resp.data.conversations]);
            })
            .catch(() => {});
    };

    const fetchMessages = async (convId: number, page: number) => {
        await api.get("/Support/Messages", { params: { page: page, conversationId: convId } })
            .then(resp => {
    ...
    const endConversation = async () => {
        ...
        await api.put("/Support/CloseConversation", { conversationId: currentConv.id })
            .then(() => {})
            .catch(() => {});
        setCurrentConv(null);
    ...
    const sendMessage = async () => {
        ...
        if(connectionRef.current)
            await connectionRef.current.send("SendMessage", currentConv.id, msgInput.trim() );
    };
    ...
    useEffect(() => {
        if(!currentConv && newConv) {
            ...
            if(ind !== -1)
                handleConvClick(conversations[ind]);
        }
    }, [conversations]);
        ...
        <div className="support">
            <div className="support-left">
                <div className="support-tabs">
                    <div className="support-tab" onClick={() => changeTab(0)}>Active</div>
                    <div className="support-tab" onClick={() => changeTab(1)}>Closed</div>
                ...
                <div className="support-list">
                    {conversations.map((conv, ind) => (
                        <div 
                            key={`conv-${ind}`} 
                            className={`support-item ${conv.id === currentConv?.id ? 'support-active-item' : ''}`} 
                            onClick={() => handleConvClick(conv)}
                        >
                            <img src={(catIcons as any)[conv.category.shortForm] ?? ''} />
                            <div className="support-subitem">
                                <h2>{conv.type}: <strong>{conv.title}</strong> <i>#{conv.id}</i></h2>
                                <p>{conv.lastSender}: {conv.lastMessage.substring(0, 30)}{conv.lastMessage.length > 30 ? '...' : ''}</p>
                            </div>
                        </div>
                    ))}
                    {totalConvPages > convPage && <div className="support-load-more" onClick={loadMoreConversations}>
                        LOAD MORE
                    </div>}
                </div>
            </div>
            <div className="support-right">
                {!currentConv && <div className="support-info">
                ...
                <div className="support-header">
                    ...
                    {!disabled && <div className="support-end" onClick={() => endConversation()}>END</div>}
                </div>}
                <div className="support-content">
                    {(currentConv || isNew) && <div className={`support-input-con ${disabled ? 'support-disabled' : ''}`}>
                        <AutoGrowTextarea
                            value={msgInput}
                    ...
                    {currentConv && <div className="support-messages">
                        {messages.map((msg, ind) => 
                        <div key={`msg-${ind}`} className={`support-message ${tokenData?.userId === msg.senderId ? 'support-message-right' : ''}`}>
                            <Link to={`/user/${msg.senderId}`}><AuthImage src={`/user/${msg.senderId}/ProfilePicture`} element={Avatar} /></Link>
                            <div className="support-message-data">
                                <div className="support-message-sender">
                                    <Link to={`/user/${msg.senderId}`}><div className="support-name">
                                        <span className={`role-${msg.senderRole.toLocaleLowerCase()}`}>{msg.senderRole !== 'User' ? msg.senderRole : ''}</span>{msg.senderUsername}
                                    </div></Link>
                                    <div className="support-date">{format(new Date(msg.sentAt), 'dd/MM/yyyy HH:mm:ss')}</div>
                                </div>
                                <div className="support-message-content">{msg.content}</div>
                            </div>
                        </div>)}
                        {totalMsgPages > msgPage && <div className="support-load-more" onClick={loadMoreMessages}>
                            LOAD MORE
                        </div>}
                    </div>}
                </div>
