...
function CreateCourse() {
  ...
  const [currentType, setCurrentType] = useState<"challenge" | "lesson" | null>(
    null
  );
    ...
    api
      .get("/Challenge/Search", {
        params: {
          ...
          exclude: course.items.filter((i) => i.type === 0).map((i) => i.id),
        },
      })
      .then((res) => {
  ...
  const fetchLessons = () => {
    api
      .get("/Lesson/Search", {
        params: {
          search: searchLesson,
          exclude: course.items.filter((i) => i.type === 1).map((i) => i.id),
        },
      })
      .then((res) => {
    ...
    if (currentItem !== null && currentType) {
      if (currentType == "challenge") {
        ...
        setCourse((prev) => ({
          ...
          items: insertAt(
            prev.items,
            currentMyItem ?? prev.items.length,
            newItem
          ),
        }));
        ...
        setCourse((prev) => ({
          ...
          items: insertAt(
            prev.items,
            currentMyItem ? currentMyItem + 1 : prev.items.length,
            newItem
          ),
        }));
  ...
  const createCourse = () => {
    ...
    validationSchema
      ...
      .then(() => {
        ...
        if (
          preview &&
          bannerFileRef.current?.files &&
          bannerFileRef.current.files.length > 0
        )
        ...
        api
          .post("/Course/CreateCourse", formData)
          .then((resp) => {
    ...
    validationSchema
      ...
      .then(() => {
        ...
        if (
          ...
          formData.append("Banner", bannerFileRef.current.files[0]);
        else if (!course.hasBanner) formData.append("DeleteBanner", "true");
        ...
        api
          .put("/Course/UpdateCourse", formData)
          .then(() => {
  ...
  useEffect(() => {
    if (params.id) {
      api
        .get(`/Course/CourseDetails/${params.id}`)
        .then((res) => {
          setCourse(res.data);
          setTriggerFetch(true);
        })
        .catch((err) => {
          if (err.response.status == 404) navigate("/moderator/courses");
  ...
  return (
    <div className="studio-create" ref={containerRef}>
      ...
      <form className="studio-create-form">
          ...
          <div className="studio-banner-upload">
            <div className="studio-upload-con">
              <div className="studio-upload-subcon">
                ...
                >
                  <MdCloudUpload /> Upload Banner
                </button>
              </div>
              {(course.hasBanner || preview) && (
                <MdDelete
            ...
            <div className="studio-banner-preview">
              ...
              {(course.hasBanner || preview) &&
                (course.hasBanner && course.id ? (
                  <AuthImage
                    src={
                      course.hasBanner && course.id !== undefined
        ...
        <h2>Course Lessons & Challenges</h2>
          ...
          <div className="studio-course-col">
            ...
            <div className="studio-course-items">
              {challenges.map((challenge, ci) => (
                <div
                  className={`studio-course-item ${
                    currentItem === ci && currentType === "challenge"
                      ? "studio-course-active"
                      : ""
                  }`}
                  key={ci}
                  onClick={() => {
                ...
                >
                  <img
                    src={(categories as any)[challenge.category.shortForm]}
                  />
                  <div className="studio-course-info">
                    ...
                    <p>
                      ...
                      <span
                        style={{ color: getColorHex(challenge.difficulty) }}
                      >
            ...
            <Link to="/challenges" target="_blank">
              All Challenges
            </Link>
          </div>
          <div className="studio-course-col">
            ...
            <div className="studio-course-items">
              {lessons.map((lesson, ci) => (
                <div
                  className={`studio-course-item ${
                    currentItem === ci && currentType === "lesson"
                      ? "studio-course-active"
                      : ""
                  }`}
                  key={ci}
                ...
                >
                  ...
                  <div className="studio-course-info">
                    ...
                    <p>
                      ...
                      <span style={{ color: getColorHex(lesson.difficulty) }}>
            ...
            <Link to="/lessons" target="_blank">
              All Lessons
            </Link>
          </div>
          <FaAngleDoubleRight onClick={addItemToList} />
          <div className="studio-course-myitems">
            {course.items.map((item, ii) => (
              <div
                className={`studio-course-myitem ${
                  currentMyItem === ii ? "studio-course-active" : ""
                }`}
                key={ii}
                onClick={() => {
              ...
              >
                ...
                <div className="studio-course-info">
                  <h2>
                    {["Challenge: ", "Lesson: "][item.type]}
                    {item.name}
                  </h2>
                  <p>
                    {item.categoryName} -{" "}
                    <span style={{ color: getColorHex(item.difficulty) }}>
                ...
                <div className="studio-course-moving">
                  ...
                  <FaRegCircleDown
                    className={
                      ii === course.items.length - 1
                        ? "studio-move-disabled"
                        : ""
                    }
                    onClick={(e) => {
                      e.stopPropagation();
          ...
          {!loading && (
            <>
              {!params.id && (
                <button
                  type="button"
                  className="studio-btn-add"
                  onClick={createCourse}
                >
                  <FaPlus className="studio-icon" /> Create
                </button>
              )}
              {params.id && (
                <>
                  ...
                  >
                    <IoIosSave className="studio-icon" /> Save Changes
                  </button>
                  {tokenData?.role === "Admin" && (
                    <button
                      type="button"
                      className="studio-btn-del"
                      onClick={deleteCourse}
                    >
                      <FaTrashAlt className="studio-icon" /> Delete
                    </button>
                  )}
                </>
              )}
