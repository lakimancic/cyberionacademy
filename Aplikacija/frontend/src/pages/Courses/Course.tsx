    ...
    <div className="course-con">
      <div className="course-header">
        <div className="course-header-info">
          <h1>{course.title}</h1>
          <h2 style={{ color: getColorHex(course.difficulty) }}>
        ...
        <div className="course-items-list">
          ...
          {course.items.map((item, ci) => (
            <div
              ...
              key={ci}
              onClick={() =>
                navigate(`/${["challenges", "lessons"][item.type]}/${item.id}`)
              }
            >
              <h2>
                {["Challenge", "Lesson"][item.type]}: {item.name}
              </h2>
              <p>
        ...
        <div className="course-right">
          ...
          >
            ...
            <div className="author-info">
              <AuthImage
                src={`/User/${course.authorId}/ProfilePicture`}
                element={Avatar}
              />
              <div className="author-details">
                <p
                  className={`author-role role-${course.authorRole?.toLowerCase()}`}
                >
                  {course.authorRole}
                </p>
              ...
              <div className="rating-item">
                ...
                {course.reviewCount > 0 && (
                  <div className="rating-counts">
                    {course.difficultyCounts.map((diff, i) => {
                      const maxCount = course.difficultyCounts
                        .map((dc) => dc.count)
                        .reduce((max, current) =>
                ...
                <div
                  ...
                  style={{
                    color: getColorHex(
                      Math.round(
                        course.averageReviewDifficulty ?? course.difficulty
                      )
                    ),
                  }}
                >
