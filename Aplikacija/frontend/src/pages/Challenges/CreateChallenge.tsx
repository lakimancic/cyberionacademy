...
function CreateChallenge() {
  ...
  const fetchModChallenge = () => {
    api
      ...
      .catch((err) => {
        if (err.response.status == 404) navigate("/moderator/challenges");
    ...
    validationSchema
      ...
      .then(() => {
        ...
        formData.append(
          "CategoryId",
          (
            categories.find((c) => c.name === challenge.categoryName)?.id ?? 0
          ).toString()
        );

        if (
          downloadFileRef.current?.files &&
          downloadFileRef.current.files.length > 0
        )
          formData.append("DownloadFile", downloadFileRef.current.files[0]);

        if (
          dockerFileRef.current?.files &&
          dockerFileRef.current.files.length > 0
        )
          formData.append("DockerFile", dockerFileRef.current.files[0]);

        api
          .post("/Challenge/CreateChallenge", formData)
          .then((resp) => {
  ...
  const updateChallenge = () => {
    ...
    validationSchema
      ...
      .then(() => {
        ...
        if (!challenge.dockerFile) formData.append("DeleteDockerFile", "true");
  ...
  const deleteDockerFile = () => {
    ...
    if (dockerFileRef.current) dockerFileRef.current.value = "";
  ...
  return (
    <div className="studio-create">
      <IoMdArrowRoundBack
        ...
        onClick={() => navigate("/moderator/challenges")}
      />
        ...
        <div className="studio-create-col">
          ...
          <div className="form-field">
            ...
            <Slider
              ...
              max={
                pointsValues[challenge.difficulty].start +
                pointsValues[challenge.difficulty].step * 4
              }
            />
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) =>
                      handleInputChange(e.target.checked, "isPublic")
                    }
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) =>
                      handleInputChange(!e.target.checked, "isArchived")
                    }
                  />
                }
            ...
            <Select
              ...
              renderValue={(selected) => {
                if (selected.length === 0)
                  return (
                    <span className="admin-placeholder">Select Category</span>
                  );
        ...
        <div className="studio-create-col">
          <div className="form-field">
            <div className="form-label">
              Docker Zip File (Containing Dockerfile)
            </div>
            <div className="studio-upload-con">
              <div className="studio-upload-subcon">
                <button
                  type="button"
                  onClick={() => dockerFileRef.current?.click()}
                >
                  <MdCloudUpload /> Upload
                </button>
                {(!!dockerFileName || !!challenge.dockerFile) && (
                  <MdDelete
                    onClick={deleteDockerFile}
                    className={`studio-upload-remove ${
                      challenge.dockerImage ? "studio-upload-disabled" : ""
                    }`}
                  />
                )}
              </div>
          ...
          <div className="form-field">
            ...
            <div className="studio-upload-con">
              <div className="studio-upload-subcon">
                ...
                {(!!downloadFileName || !!challenge.downloadFile) && (
                  <MdDelete
          ...
          <div className="studio-docker-con">
            ...
            <div className="studio-docker-buttons">
              <button
                type="button"
                className="studio-btn-add"
                disabled={!!challenge.dockerImage}
                onClick={buildDocker}
              >
                <IoMdBuild /> Build
              </button>
