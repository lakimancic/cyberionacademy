...
function SearchPage() {
    ...
    return (
        <div className="search-page">
            ...
            {challenges.length > 0 && <>
                <h2><Link to="/challenges">Challenges</Link></h2>
                <div className="search-items-grid">
                    {challenges.map((c, i) => (
                        <div className="search-item" key={i} onClick={() => navigate(`/challenges/${c.id}`)}>
                            <img
                                className="cat-icon"
                            ...
                            <div className="search-item-name">
                                ...
                                <div className="difficulty" style={{ color: getColorHex(c.difficulty) }}>
            ...
            {lessons.length > 0 && <>
                <h2><Link to="/challenges">Lessons</Link></h2>
                <div className="search-items-grid">
                    {lessons.map((c, i) => (
                        <div className="search-item" key={i} onClick={() => navigate(`/lessons/${c.id}`)}>
                            <img
                                className="cat-icon"
                                src={(categoriesIcon as any)[c.category.shortForm]}
                            />
            ...
            {users.length > 0 && <>
                <h2><Link to="/leaderboard">Users</Link></h2>
                <div className="search-items-grid">
                    {users.map((u, i) => (
                        <div className="search-item" key={i} onClick={() => navigate(`/user/${u.id}`)}>
                            <AuthImage 
            ...
            courses.length === 0 && users.length === 0 && <div className='search-no-result'>
                No Results
            </div>}
        </div>
    )
