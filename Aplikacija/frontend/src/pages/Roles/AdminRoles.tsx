...
function AdminRoles() {
    ...
    const fetchRoleRequests = async (isPending: boolean) => {
        await api.get("/Roles/RoleRequests", { params: { page: currentPage, pending: isPending }})
            .then(resp => {
                setTotalPages(resp.data.totalPages);
    ...
    const submitRoleChange = (userInd : number) => {
        ...
        api.put("/Roles/ChangeUserRole", { newRole: user.role, userId: user.userId })
            .then(() => {
                fetchUsers();
            })
            .catch(() => {})
    ...
    const acceptReject = (accept: boolean) => {
        ...
        api.put("/Roles/AcceptRejectRole", { accept: accept, requestId: roleRequest?.id })
            .then(() => {
                setCurrentPage(1);
                setTotalPages(1);
    ...
    return (
        <div className="admin-roles">
            ...
            <div className="admin-content">
                {tab < 2 && roleRequest === null && <DataTable 
                    data={requests.map(req => {
                        return {
                            username: req.username,
                            requestedAt: (new Date(req.requestedAt)).toLocaleDateString(),
                            role: <strong className={`role-${req.role.toLocaleLowerCase()}`}>{req.role}</strong>,
                            status: req.status
                        }
                    })}
                    columns={[
                ...
                />}
                {tab < 2 && roleRequest !== null && <div className='admin-role-request'>
                    <IoMdArrowRoundBack className='admin-back' onClick={() => setRoleRequest(null)} />
                    <div className="admin-req-left">
                        <div className="admin-req-user">
                            <Link to={`/user/${roleRequest.userId}`}>
                                <AuthImage src={`/User/${roleRequest.userId}/ProfilePicture`} element={Avatar} className='admin-req-avatar' />
                            </Link>
                            <div className="admin-req-user-info">
                                <div className="admin-req-username">{roleRequest.username}</div>
                                <div className='admin-req-role'>Requsted Role: <span className={`role-${roleRequest.role.toLocaleLowerCase()}`}>{roleRequest.role}</span></div>
                                <div className="admin-req-date">Requested at: <strong>{(new Date(roleRequest.requestedAt)).toLocaleDateString()}</strong></div>
                            </div>
                        </div>
                        {tab === 0 && <div className="admin-req-buttons">
                            <button className="admin-req-accept" onClick={() => acceptReject(true)}>
                                Accept <FaCheck />
                            </button>
                            <button className="admin-req-reject" onClick={() => acceptReject(false)}>
                                Reject <FaXmark />
                            </button>
                        </div>}
                    </div>
                ...
                {tab == 2 && <div className='admin-users-con'>
                    <div className="admin-filters">
                        <SearchBar label='Users' setSearchWord={setSearchWord} searchWord={searchWord} onSearch={onSearch} />
                        <div className="admin-filter-role">
                            ...
                            renderValue={selected => {
                                if(selected.length === 0)
                                    return <span className='admin-placeholder'>Filter by Role</span>;

                                return selected;
                            }}
                            >
                                <MenuItem value=''>All Roles</MenuItem>
                                {roles.map((role, idx) => (<MenuItem key={idx} value={role}>
                                    {role}
                                </MenuItem>))}
                            </Select>
                        </div>
                    ...
                    <DataTable 
                        data={users.map((user, ind) => {
                            return {
                                ...
                                action: <button onClick={() => submitRoleChange(ind)}>
                                    Change Role
                                </button>
                            }
                        })}
