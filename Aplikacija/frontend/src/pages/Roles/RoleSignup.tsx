...
function RoleSignup() {
  ...
  const onSubmit = (data: RoleRequest) => {
    ...
    api
      .post("/Roles/SubmitRoleRequest", data)
      .then(() => {
        updateData();
      })
      .catch((err) => {
  ...
  const updateData = () => {
    api
      .get("/Roles/GetRoleRequests")
      .then((resp) => {
        setRequests(
          resp.data.requests.map((req: any) => {
  ...
  return (
    <div className="role-signup">
      ...
      <h2 className="role-signup-current">
        Current role:{" "}
        <span
          className={`role-signup-role role-${tokenData?.role.toLocaleLowerCase()}`}
        >
          {tokenData?.role}
        </span>
      </h2>
      <div className="role-signup-tabs">
        ...
        <div
          className={`role-signup-active-${tab === 0 ? "first" : "second"}`}
        ></div>
      </div>
      <div className="role-signup-content">
        {tab === 0 && cooldown && (
          <div className="role-signup-cooldown">
            <FaRegClock />
            <h2>You can only submit request once a month!</h2>
          </div>
        )}
        {tab === 0 && tokenData?.role === "Admin" && (
          <div className="role-signup-admin">
            <GrUserAdmin />
            <h2>You are already Administrator!</h2>
          </div>
        )}
        {tab === 0 && !cooldown && tokenData?.role !== "Admin" && (
          <form className="role-signup-form" onSubmit={handleSubmit(onSubmit)}>
                ...
                render={({ field }) => (
                  <RadioGroup row {...field}>
                    {roleInd < 1 && (
                      <FormControlLabel
                        value="Helper"
                        control={<Radio className="role-radio-helper" />}
                        label="Helper"
                      />
                    )}
                    {roleInd < 2 && (
                      <FormControlLabel
                        value="Moderator"
                        control={<Radio className="role-radio-moderator" />}
                        label="Moderator"
                      />
                    )}
                    {roleInd < 3 && (
                      <FormControlLabel
                        value="Admin"
                        control={<Radio className="role-radio-admin" />}
                        label="Administrator"
                      />
                    )}
                  </RadioGroup>
            ...
            <p className="role-signup-label">
              Request letter (min 50 characters):{" "}
            </p>
        ...
        {tab === 1 && (
          <div className="role-signup-requests">
            {requests.map((req, ind) => (
              <div key={ind} className="role-signup-request">
                <div className="role-request-reqinfo">
                  <div className="role-request-role">
                    For{" "}
                    <span className={`role-${req.role.toLocaleLowerCase()}`}>
                      {req.role}
                    </span>
                  </div>
                  <div className="role-request-date">
                    Submitted at: <span>{req.requestedAt.toDateString()}</span>
                  </div>
                </div>
                <div className="role-request-status">
                  Status:{" "}
                  <span className={`role-${req.status.toLocaleLowerCase()}`}>
                    {req.status}
                  </span>
                </div>
              </div>
